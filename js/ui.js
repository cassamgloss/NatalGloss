const UI = {
    ...
    init: function() {
        ...
        // Configura volume do efeito
        this.sfxBoom.volume = 1.0;

        this.destrancarAudio();
        
        // Listeners para desbloquear áudio do navegador
        ...
        // Torna UI global acessível
        window.UI = this; 
    },

    destrancarAudio: function() { 
        if (this.jogoRolando || !this.somAtivo || this.musicaMenuTocando) return; 
    ...
    // --- INÍCIO DE FASES (2 e 3) ---
    abrirInstrucoesFase: function(nivel) {
        const bg = document.getElementById('menu-bg');
        // Define imagem de fundo correta baseado no nível (fallback se não tiver bg 3)
    ...
    // --- TEXTOS DAS INSTRUÇÕES (Com Fase 3 Inclusa) ---
    ...
    fecharInstrucoes: function() {
        ...
        // Inicia o motor do jogo (e a música) antes da contagem
        if (!window.gameScene) {
            this.iniciarPhaser();
        }
    ...
    // --- CONTAGEM REGRESSIVA (COM EFEITO GRAVE NO FINAL) ---
    rodarContagem: function() {
        ...
        // 1º BOOM (Normal)
        if(this.somAtivo) {
            this.sfxBoom.currentTime = 0;
            this.sfxBoom.playbackRate = 1.0; 
            this.sfxBoom.play().catch(()=>{});
        }
        ...
        let timer = setInterval(() => {
            ...
            if(c > 0) { 
                ...
                num.classList.remove('pop-in'); 
                void num.offsetWidth; 
                num.innerText = c; 
                num.classList.add('pop-in'); 
            }
            else if (c === 0) { 
                // 4º BOOM (EFEITO FINAL: GRAVE E LONGO)
                if(this.somAtivo) {
                    this.sfxBoom.currentTime = 0;
                    this.sfxBoom.playbackRate = 0.5; // Metade da velocidade = Mais grave
                    this.sfxBoom.play().catch(()=>{});
                }
                ...
                // Libera o movimento na cena do jogo
                if (window.gameScene) {
